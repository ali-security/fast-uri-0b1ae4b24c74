...
test('decoded schemes select their scheme handlers', (t) => {
  ...
  // The handler is looked up with the decoded scheme, so an encoded "ws:" is
  // still handed to the ws handler instead of falling through to generic
  // parsing (which would leave the resource name unset).
  const encodedWs = fastURI.parse('w%73://example.com/chat')
  t.equal(encodedWs.scheme, 'ws', 'encoded ws scheme is decoded')
  t.equal(encodedWs.resourceName, '/chat', 'the ws handler ran for the encoded scheme')
  t.equal(encodedWs.path, undefined, 'the ws handler consumed the path')
  t.end()
})
