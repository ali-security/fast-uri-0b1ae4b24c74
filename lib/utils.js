  ...
  if (component.host !== undefined) {
    ...
    if (!isIPv4(host)) {
      ...
      if (ipV6res.isIPV6 !== true && ipV6res.isIPVFuture !== true) {
        // Decode the percent escapes only once, and only after the IP-literal
        // check has failed, so that an encoded "%25" zone separator cannot
        // become the introducer for a second escape during recomposition.
        host = unescape(host)
